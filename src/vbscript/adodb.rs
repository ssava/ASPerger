...
impl VBScriptObject for Connection {
    fn type_name(&self) -> &'static str { "Connection" }
    ...
    fn get_property(&self, name: &str, _context: &mut ExecutionContext) -> Result<VBValue, VBSError> {
        match name.to_uppercase().as_str() {
            ...
            _ => Err(VBSErrorType::RuntimeError.into_error(
                format!("Property '{}' not found on Connection object", name)
            )),
        }
    }
    ...
    fn set_property(&mut self, name: &str, value: VBValue, _context: &mut ExecutionContext) -> Result<(), VBSError> {
        match name.to_uppercase().as_str() {
            ...
            _ => Err(VBSErrorType::RuntimeError.into_error(
                format!("Cannot set property '{}' on Connection object", name)
            )),
        }
    }
    ...
    fn call_method(&mut self, name: &str, args: &[VBValue]) -> Result<VBValue, VBSError> {
        match name.to_uppercase().as_str() {
            ...
            _ => Err(VBSErrorType::RuntimeError.into_error(
                format!("Method '{}' not found on Connection object", name)
            )),
        }
    }
}
...
impl VBScriptObject for Recordset {
    fn type_name(&self) -> &'static str { "Recordset" }
    ...
    fn get_property(&self, name: &str, _context: &mut ExecutionContext) -> Result<VBValue, VBSError> {
        match name.to_uppercase().as_str() {
            ...
            _ => Err(VBSErrorType::RuntimeError.into_error(
                format!("Property '{}' not found on Recordset object", name)
            )),
        }
    }
    ...
    fn call_method(&mut self, name: &str, _args: &[VBValue]) -> Result<VBValue, VBSError> {
        match name.to_uppercase().as_str() {
            "MOVENEXT" => {
                if self.eof {
                    return Err(VBSErrorType::RuntimeError.into_error(
                        "Cannot move past EOF".to_string()
                    ));
                }
            ...
            _ => Err(VBSErrorType::RuntimeError.into_error(
                format!("Method '{}' not found on Recordset object", name)
            )),
        }
    }
}
